...
프로토타입 체인에 의한 상속으로 인해 해당 인스턴스는 표준 빌트인 생성자함수의 프로토타입이 가지는 프로토타입 프로퍼티와 메소드를 자유롭게 사용할 수 있다.
...
encodeURIComponent 함수는 매개변수로 전달된 문자열을 URI의 구성요소인 쿼리 파라미터의 일부로 간주한다. 따라서 쿼리파라미터 구분자로 사용되는 = / ? / &를 인코딩한다.
